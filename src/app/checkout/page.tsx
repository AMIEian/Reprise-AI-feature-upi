...
// Replace react-router-dom usages with Next.js equivalents to avoid missing module error
...
export default function Checkout() {
  ...
  // provide minimal replacements for navigate and location used in this file
  const navigate = (path: string) => router.push(path);
  const location = { state: undefined as any };
  const { user } = useAuth();
  ...
  const applyOrRemoveCoupon = async () => {
    ...
    try {
      ...
      const res = await api.post(`/sell-phone/coupons/validate`, {
        ...
        phone_id:
          phoneData.id && phoneData.id !== "default"
            ? Number(phoneData.id)
            : null,
      });
      const data = res.data;
      if (data.valid) {
        ...
        toast.success(
          `Coupon applied! +₹${(data.amount ?? 0).toLocaleString()} bonus`,
        );
      } else {
        setCouponMsg(data.message);
        setCouponSuccess(false);
                              ...
                              <div className="grid grid-cols-7 gap-1.5">
                                {days.map((d, i) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <span
                                        className={`text-[10px] font-semibold tracking-wider ${isToday ? "text-white/90" : isSelected ? "text-blue-500" : "text-gray-400"}`}
                                      >
                                        {isToday
                                          ? "TODAY"
                                          : dayNames[d.getDay()]}
                                      ...
                                      <span
                                        className={`text-lg font-bold leading-tight ${isToday ? "text-white" : isSelected ? "text-blue-700" : "text-gray-800"}`}
                                      >
                                        {d.getDate()}
                                      </span>
                                      {isToday && (
                    ...
                    <div className="my-4">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && applyOrRemoveCoupon()
                          }
                      ...
                      {couponMsg && (
                        <p
                          className={`text-xs mt-1 font-medium ${couponSuccess ? "text-green-600" : "text-red-500"}`}
                        >
                          {couponSuccess ? "✓ " : "✗ "}
                          {couponMsg}
                        </p>
                  ...
                  <div className="border-t pt-4">
                    ...
                    {couponBonus > 0 && (
                      <p className="text-xs text-green-600 font-medium mt-1">
                        🎉 You're getting ₹{couponBonus.toLocaleString()} extra
                        with coupon!
                      </p>
                    )}
